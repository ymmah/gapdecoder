...
class Decrypter:
    ...
    def get_new_bytes(self, bytes, new_bytes, index):
        for i in range(16):
            self.j[i % 4][i // 4] = bytes[index + i]

        self.map_j_to_C(10)
        for a in range(1, 10):
            self.change_wc_and_set_j_to_wc()
            self.map_j_to_C(10-a)
            f = self.wc[0]
            for g in range(4):
                f[0] = self.j[0][g]
                f[1] = self.j[1][g]
                f[2] = self.j[2][g]
                f[3] = self.j[3][g]
                self.j[0][g] = magic_4[f[0]] ^ magic_2[f[1]] ^ magic_3[f[2]] ^ magic_1[f[3]]
                self.j[1][g] = magic_1[f[0]] ^ magic_4[f[1]] ^ magic_2[f[2]] ^ magic_3[f[3]]
                self.j[2][g] = magic_3[f[0]] ^ magic_1[f[1]] ^ magic_4[f[2]] ^ magic_2[f[3]]
                self.j[3][g] = magic_2[f[0]] ^ magic_3[f[1]] ^ magic_1[f[2]] ^ magic_4[f[3]]

        self.change_wc_and_set_j_to_wc()
        self.map_j_to_C(0)

        for a in range(16):
            new_bytes.append(self.j[a % 4][a // 4])
        return new_bytes
    
    def map_j_to_C(self, b):
        for c in range(4):
            for d in range(4):
                self.j[c][d] = self.j[c][d] ^ self.C[4 * b + d][c]


    def change_wc_and_set_j_to_wc(self):
        for b in range(1, 4):
            for c in range(4):
                self.wc[b][(c+b) % 4] = self.j[b][c]
        for b in range(1, 4):
            for c in range(4):
                self.j[b][c] = self.wc[b][c]
        for b in range(4):
            for c in range(4):
                self.j[b][c] = m_Rc[self.j[b][c]]
